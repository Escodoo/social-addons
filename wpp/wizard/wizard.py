...
class SendMessage(models.TransientModel):
    ...
    def onchange_template_id_wrapper(self):
        ...
        values = self.onchange_template_id(self.template_id.id, self.model, res_id)[
            "value"
        ]
    ...
    def onchange_template_id(self, template_id, model, res_id):
        if template_id:
            ...
            default_values = self.with_context(
                default_model=model, default_res_id=res_id
            ).default_get(["model", "res_id", "partner_ids", "message"])
            values = {
