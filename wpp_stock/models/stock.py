...
class InventoryTransferDone(models.Model):
    ...
    def inventory_wpp(self):
        ...
        if not record_phone[0] == "+":
            ...
            context[
                "message"
            ] = "No Country Code! Please add a valid mobile \
    ...
    def send_direct_message(self):
        ...
        if not record_phone[0] == "+":
            ...
            prods = ""
            for rec in self:
                for rec in rec.move_line_ids_without_package:
                    prods = (
                        prods
                        + "*"
            ...
            link = "https://web.wpp.com/send?phone=" + ph_no
    ...
    def multi_sms(self):
        ...
        if cust_check:
            ...
            for each in inventory_order_ids:
                prods = ""
                for rec in each.move_line_ids_without_package:
                    prods = (
                        prods + "*" + "Product: " + str(rec.product_id.name) + "* \n"
                    )
